...
| # | Action | Effort | Critère d'acceptation | Statut 2026-07-19 |
|---|---|---|---|---|
| 2.1 | **Tarifs par région** au catalog (plus de « US East pour tout le monde ») + devise source affichée | M | Un insight eu-west-3 cite la grille eu-west-3, datée | **FAIT** : grilles EBS et Extended Support RDS par région (us-east-1/eu-west-1/eu-west-3, sourcées AWS + Price List API) ; découverte : l'ES RDS n'est **pas** uniforme (+12-18 % eu-west) — appliquée aux 3 règles RDS via le fact `aws.rds.region` ; `price_region_exact` dans tous les payloads |
